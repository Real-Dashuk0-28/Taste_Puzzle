...
class MainWindow(QMainWindow):
    ...
    def load_ingredients_for_autocomplete(self):
        """Загружает ингредиенты для автодополнения"""
        try:
            ingredients = self.db.get_ingredients()
            ingredient_names = [name for _, name in ingredients]

            # Очищаем и добавляем ингредиенты в выпадающий список
            self.ingredient_filter.clear()
            self.ingredient_filter.addItems([""] + sorted(ingredient_names))

            # Настраиваем автодополнение
            completer = QCompleter(ingredient_names, self)
            completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
            completer.setFilterMode(Qt.MatchFlag.MatchContains)
            completer.setCompletionMode(QCompleter.CompletionMode.PopupCompletion)

            self.ingredient_filter.setCompleter(completer)

            logger.info(f"Загружено {len(ingredient_names)} ингредиентов для автодополнения")
    ...
    def add_recipe(self):
        ...
        try:
            ...
            dialog.recipe_saved.connect(self.load_ingredients_for_autocomplete)  # Добавляем эту строку
            dialog.exec()
        except Exception as e:
